...
class InstallCommontator < ActiveRecord::Migration[6.0]
  def change
    ...
    add_index :commontator_comments,
              %i[creator_id creator_type thread_id],
    ...
    add_index :commontator_subscriptions,
              %i[subscriber_id subscriber_type thread_id],
              unique: true,
              name:
                'index_commontator_subscriptions_on_s_id_and_s_type_and_t_id'
  end
end
